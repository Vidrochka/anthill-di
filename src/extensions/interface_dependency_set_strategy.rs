...
pub trait InterfaceDependencySetStrategy {
    ...
    async fn set_singleton_interface<TInterface: ?Sized + 'static, TType: Constructor + Unsize<TInterface>>(&self) -> AddDependencyResult<()>;
    async fn set_scoped_interface<TInterface: ?Sized + 'static, TType: Constructor + Unsize<TInterface>>(&self) -> AddDependencyResult<()>;
}
